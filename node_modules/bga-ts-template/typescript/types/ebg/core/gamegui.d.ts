...
declare class Gamegui extends SiteCore {
	...
	/** Safer way of accessing this.prefs[pref_id].value */
	getGameUserPreference(pref_id: number): number | `${number}` | undefined
	...
	setGameUserPreference(pref_id: number, value: number|string): void
	 ...
	 * @see https://en.doc.boardgamearena.com/Game_interface_logic:_yourgamename.js#Title_bar
	 */
	statusBar: {
		/**
		 * Updates the page title, without triggering onUpdateActionButtons. Can handle ${you} and ${actplayer} and any other var you would pass as args.
		 * @param title The new title.
		 * @param args Optional arguments to substitute in the title.
		 */
		setTitle(title: string, args?: any): void;

		/**
		 * Adds an action button to the status bar.
		 * @param label The text of the button. Should be translatable (use _() function).
		 * @param callback The function to call when the button is clicked.
		 * @param options Optional parameters for the button.
		 * @returns The generated DOM element if you want to manipulate it afterwards.
		 */
		addActionButton(label: string, callback: () => void, options?: {
			/** The color of the button. 'secondary' will be clearer for new devs than a gray option that displays a white button. */
			color?: 'primary' | 'secondary' | 'red';
			/** Additional classes to apply to the button, for example to set the button disabled. */
			classes?: string[];
			/** The DOM element to which the button should be appended, in case you want to place an action button outside of the status bar. */
			destination?: HTMLElement;
			/** An alternative text for buttons containing only an icon, for accessibility. */
			title?: string;
		}): HTMLElement;

		/**
		 * Removes all action buttons on the status bar.
		 */
	...
	setClientState<T extends keyof ClientGameStates>(newState: T, args: {descriptionmyturn: string, args?: ClientGameStates[T]}): void;
	...
	/**
	 * Triggers an action on the backend. This is a combination of checkAction and ajaxcall, returning a Promise which resoves when ajaxcall ends.
	 * 
	 ...
	 * @param action name of the action, as it is written in "possibleactions" of the current state.
	 * @param args an object containing the call parameters to send to the action
	 * @param options options to tweak the call with some defaults. Default is { lock: true, checkAction: true }.
	 * @returns a Promise which resoves when ajaxcall ends.
	 * @example
	 ...
	 * this.bgaPerformAction('actChangeMind', {}, { checkAction: false });
	 * @example
	 ...
	 */
	bgaPerformAction<K extends keyof PlayerActions>(
		action: K, 
	...
	/**
	 * @param params see https://boardgamearena.com/doc/Game_interface_logic:_yourgamename.js
	 */
    bgaSetupPromiseNotifications(params: { 
      prefix?: string,					// 'notif_'
      minDuration?: number,				// 500
      minDurationNoText?: number,		// 1
      handlers?: Function[],			// []
      logger?: Function,				// null
      ignoreNotifications?: string[],	// []
      onStart?: (notifName: string, msg: string, args: object) => void, 
      onEnd?: (notifName: string, msg: string, args: object) => void,
    }): void
}
