	...
	builder.watchCommand("Game States: 'gamestates.jsonc' => 'states.inc.php', '.action.php', [gamestates.d.ts]", 'source/shared/gamestates.jsonc', () => {
		...
		for (const key in statesJSON) {
			...
			for (const transition in transitions) {
				...
				const target = transitions[transition].toString();
				if (!stateIds.has(target) && target !== '99' && target !== '1') {
...
$machinestates = ${writer.stringify(statesJSON, true)};`
		...
		fs.writeFileSync('daleofmerchants.action.php',
`<?php
${writer.fileSignature}
...
if (fs.existsSync('source/client/tsconfig.json')) {
	...
	const tsCommand = 'npx tsc -p source/client --outFile daleofmerchants.js' + (builder.watch ? ' -w' : '');
	builder.execCommand(`Typescript compiler: client/define.ts => daleofmerchants.js`, tsCommand);

	// const cookbookModuleRegex = /"@cookbook\/([^"]+)"/g;
	// const searchString = 'define("daleofmerchants", [';
	// const importFixer = () => {
	// 	if (!fs.existsSync('daleofmerchants.js'))
	// 		return;

	// 	const data = fs.readFileSync('daleofmerchants.js', 'utf8');
	// 	const start = data.indexOf(searchString);
	// 	if (start !== -1) {
	// 		const end = start + searchString.length;
	// 		const result = data.substring(0, start) + 'define([' + data.substring(end);
	// 		fs.writeFileSync('daleofmerchants.js', result, 'utf8');
	...
	// builder.watchCommand("Import Generator: source/client/build/imports.d.ts", 'daleofmerchants.js', importFixer);
}

if (fs.existsSync('source/client/daleofmerchants.scss')) {
	builder.execCommand(`SCSS compiler: client/daleofmerchants.scss => daleofmerchants.css`, `npx sass --no-source-map source/client/daleofmerchants.scss daleofmerchants.css` + (builder.watch ? ' --watch' : ''));
}
