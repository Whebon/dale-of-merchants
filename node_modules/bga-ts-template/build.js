...
$machinestates = ${writer.stringify(statesJSON, true)};`
		...
		fs.writeFileSync('dale.action.php',
`<?php
${writer.fileSignature}
...
if (fs.existsSync('source/client/tsconfig.json')) {
	...
	const tsCommand = 'npx tsc -p source/client --outFile dale.js' + (builder.watch ? ' -w' : '');
	builder.execCommand(`Typescript compiler: client/define.ts => dale.js`, tsCommand);

	// const cookbookModuleRegex = /"@cookbook\/([^"]+)"/g;
	// const searchString = 'define("dale", [';
	// const importFixer = () => {
	// 	if (!fs.existsSync('dale.js'))
	// 		return;

	// 	const data = fs.readFileSync('dale.js', 'utf8');
	// 	const start = data.indexOf(searchString);
	// 	if (start !== -1) {
	// 		const end = start + searchString.length;
	// 		const result = data.substring(0, start) + 'define([' + data.substring(end);
	// 		fs.writeFileSync('dale.js', result, 'utf8');
	...
	// builder.watchCommand("Import Generator: source/client/build/imports.d.ts", 'dale.js', importFixer);
}

if (fs.existsSync('source/client/dale.scss')) {
	builder.execCommand(`SCSS compiler: client/dale.scss => dale.css`, `npx sass --no-source-map source/client/dale.scss dale.css` + (builder.watch ? ' --watch' : ''));
}
