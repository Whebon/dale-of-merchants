...
class DaleOfMerchants extends Gamegui
{
	...
	override onUpdateActionButtons(stateName: GameStateName, args: AnyGameStateArgs | null): void
	{
		...
		switch( stateName )
		{
			...
			case 'client_DEPRECATED_whirligig':
				if (this.unique_opponent_id) {
					this.addActionButton("confirm-button", _("Discard all"), "onDEPRECATED_Whirligig"); //only confirm discard order
				}
				else {
					this.addActionButtonsOpponentSelection(1);
					this.addActionButton("confirm-button", _("Confirm"), "onDEPRECATED_Whirligig"); //confirm opponent and discard order
				}
				this.addActionButtonCancelClient();
				break;
			...
			case 'chameleon_flexibleShopkeeper':
				...
				break;
			case 'chameleon_reflection':
				// TODO: action buttons for the optional discard effect
				this.addActionButtonCancelClient();
				break;
			case 'chameleon_goodoldtimes':
				// TODO: action buttons for the optional discard effect
			...
			case 'client_siesta':
				...
				//this.addActionButton("skip-button", _("Skip"), "onSiestaSkip", undefined, false, DaleOfMerchants.ACTION_BUTTON_SKIP);
				this.addActionButtonCancelClient();
				break;
			case 'client_ruthlessCompetition':
				this.addActionButtonCancelClient();
			...
			case 'DEPRECATED_cheer':
				...
				if (!this.isSpectator && this.myDeck.size > 0) {
					const DEPRECATED_cheer_args = args as { _private: { cards: DbCard[] } };
					this.myDeck.setContent(DEPRECATED_cheer_args._private.cards.map(DaleCard.of));
					this.myDeck.setSelectionMode('single');
				}
				break;
			case 'client_raffle':
			...
			case 'anotherFineMess':
				...
				if (looseMarbles_args.die_value1 == DaleDie.DIE_HAND) {
					...
					}).bind(this), true, TranslatableStrings.players_hand);
				}
					...
					this.addActionButtonsOpponent(((opponent_id: number) => this.showMessage(looseMarbles_fail_message, "error")).bind(this), true, TranslatableStrings.players_hand);
				}
				break;
			case 'coffeeGrinder':
	...
	*/
	 ...
	 */
	getChameleonTargets(card: DaleCard, type_id?: number): (DaleCard | HTMLElement)[] {
	...
	*/
	 ...
	 * @param label (optional) - default `"${player_name}"` - label on the button per player.
	 * @param player_ids (optional) if specified, only use these player ids. should be a subset of `this.gamedatas.playerorder`
	 */
	addActionButtonsOpponent(onOpponentHandler: (opponent_id: number) => void, include_player: boolean = false, label: string = "", player_ids?: number[]) {
		player_ids = player_ids ?? this.gamedatas.playerorder;
		if (!label) { //label could be undefined, in which case, it needs to be "${player_name}" instead
			label = "${player_name}"
		}
		for(let opponent_id of player_ids) {
			if (include_player || opponent_id != this.player_id) {
				...
				const label_formatted = `<span style="font-weight:bold;color:#${color};">${label.replace("${player_name}", name)}</span>`;
				this.addActionButton("opponent-selection-button-"+opponent_id, label_formatted, () => {onOpponentHandler(+opponent_id)}, undefined, false, DaleOfMerchants.ACTION_BUTTON_SELECT_PLAYER);
			}
		}
	}
	...
	getColourSwapHandTargets(colourSwap_card_id: number): DaleCard[] {
		...
		for (let item of this.myHand.getAllItems()) {
			const handCard = new DaleCard(item.id)
	...
	*/
	...
	onSelectHandCard(card_id: number) {
		...
		switch(this.gamedatas.gamestate.name) {
			...
			case 'client_rottenFood':
				...
				this.removeActionButtons();
				this.addActionButtonsOpponent((opponent_id: number) => {this.onRottenFood(card.id, opponent_id)}, false, _("${player_name}\'s deck"));
				this.addActionButtonCancelClient();
				break;
			case 'dirtyExchange':
			...
			case 'client_manufacturedJoy':
				...
				//alternative selection mode (bga button)
				this.removeActionButtons();
				this.addActionButtonsOpponent((opponent_id: number) => {this.onManufacturedJoy(card.id, opponent_id)}, true, _("${player_name}\'s discard"));
		...
		switch(this.gamedatas.gamestate.name) {
			...
			case 'nightShift':
				if (!TargetingLine.exists()) {
					...
					this.removeActionButtons();
					this.addActionButtonsOpponent((opponent_id: number) => {this.onNightShift(card.id, opponent_id)}, true, _("${player_name}\'s deck"), nightShift_args.player_ids);
				}
				break;
			case 'delightfulSurprise':
			...
			case 'shakyEnterprise':
				...
				//alternative selection mode (bga button)
				this.addActionButtonsOpponent((opponent_id: number) => {this.onAnchor(card.id, opponent_id)}, true, _("${player_name}\'s discard"));
	...
	/**
	 * @param target_id target card to bind to. If 0, the optional passive will be used instead.
	 * @returns 
	 */
	...
	onDEPRECATED_WhirligigDoneLooking() {
	...
	onVelocipedeToss() {
		this.playTechniqueCardWithServerState<'client_velocipede'>({
			toss: true
		});
	}

	onVelocipedeTossSkip() {
		this.playTechniqueCardWithServerState<'client_velocipede'>({
			toss: false
		});
	}

	onVelocipedeSwap(card_id: number) {
		this.bgaPerformAction('actVelocipede', {
			card_id: card_id
		});
	}

	onVelocipedeSwapSkip() {
	...
	*/
	...
	notif_buildStack(notif: NotifAs<'buildStack'>) {
		...
		(this as any).bga.playerPanels.getScoreCounter(notif.args.player_id)?.toValue(notif.args.stack_index_plus_1);
	...
	notif_DEPRECATED_whirligigShuffle(notif: NotifAs<'DEPRECATED_whirligigShuffle'>) {
	...
	notif_DEPRECATED_whirligigTakeBack(notif: NotifAs<'DEPRECATED_whirligigTakeBack'>) {
		...
		if (!this.isSpectator) {
			...
			if (notif.args._private) {
				...
				if (cards.length != notif.args.nbr) {
					throw new Error(`DEPRECATED_whirligigTakeBack failed: expected ${notif.args.nbr} cards, got ${cards.length} cards`)
				}
