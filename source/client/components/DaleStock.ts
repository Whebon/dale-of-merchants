...
export class DaleStock extends Stock implements DaleLocation {
	...
	public setClickable(card_id: number) {
		...
		if (!div) {
			throw new Error(`Card ${card_id} does not exist in ${this.control_name}, so setClickable cannot be set`);
		}
	...
	/**
	 * @returns all cards in this stock represented as DaleCard
	 */
	public getAllDaleCards(): DaleCard[] {
		const cards = [];
		for (let item of this.items) {
			cards.push(new DaleCard(item.id));
		}
		return cards;
	}
